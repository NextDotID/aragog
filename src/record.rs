...
use crate::{AragornServiceError, DatabaseConnectionPool, DatabaseRecord};

/// The main trait of the Aragorn library.
...
pub trait Record {
    ...
    async fn find(key: &str, db_pool: &DatabaseConnectionPool)
                  -> Result<DatabaseRecord<Self>, AragornServiceError>
    ...
    async fn get_where(conditions: Vec<&str>, db_pool: &DatabaseConnectionPool)
                          -> Result<Vec<DatabaseRecord<Self>>, AragornServiceError>
        where Self: DeserializeOwned + Serialize + Clone {
